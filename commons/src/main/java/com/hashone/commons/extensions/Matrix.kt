...
fun Matrix.animateScaleToPoint(
    scaleFactor: Float, dx: Float, dy: Float, onUpdate: () -> Unit = {}
) {

    val targetMatrix = this.clone().apply {
...
fun Matrix.animateToMatrix(
    targetMatrix: Matrix, onUpdate: () -> Unit = {}
) {
    ...
    translateYAnimator.addUpdateListener {
        reset()
        preScale(
            scaleAnimator.animatedValue as Float, scaleAnimator.animatedValue as Float
        )
        postTranslate(
            translateXAnimator.animatedValue as Float, translateYAnimator.animatedValue as Float
        )
        onUpdate.invoke()
    }
    ...
    AnimatorSet().apply {
            playTogether(
                scaleAnimator, translateXAnimator, translateYAnimator
            )
        }.apply { interpolator = AccelerateDecelerateInterpolator() }.apply { duration = 300 }
        .start()
