...
public class FlexboxLayoutManager extends RecyclerView.LayoutManager implements FlexContainer,
    ...
    @Override
    @NonNull
    public ArrayList<FlexLine> getFlexLines() {
        ArrayList<FlexLine> result = new ArrayList<>(mFlexLines.size());
    ...
    public void onNewFlexItemAdded(View view, int index, int indexInFlexLine, FlexLine flexLine) {
        ...
        if (isMainAxisDirectionHorizontal()) {
            ...
            flexLine.setMDividerLengthInMainSize(flexLine.getMDividerLengthInMainSize() + decorationWidth);
        } else {
            ...
            flexLine.setMDividerLengthInMainSize(flexLine.getMDividerLengthInMainSize() + decorationHeight);
        }
    }
    ...
    @Override
    public void setFlexLines(ArrayList<FlexLine> flexLines) {
        mFlexLines = flexLines;
    }

    @Override
    public ArrayList<FlexLine> getFlexLinesInternal() {
        return mFlexLines;
    }
    ...
    private void updateDirtyPosition(int positionStart) {
        ...
        assert mFlexboxHelper.getMIndexToFlexLine() != null;

        if (positionStart >= mFlexboxHelper.getMIndexToFlexLine().length) {
            return;
        }
    ...
    private void updateFlexLines(int childCount) {
        ...
        if (mDirtyPosition == NO_POSITION &&
            ...
            mAnchorInfo.mFlexLinePosition =
                    mFlexboxHelper.getMIndexToFlexLine()[mAnchorInfo.mPosition];
    ...
    private boolean updateAnchorFromPendingState(RecyclerView.State state, AnchorInfo anchorInfo,
        ...
        anchorInfo.mFlexLinePosition = mFlexboxHelper.getMIndexToFlexLine()[anchorInfo.mPosition];
    ...
    private View findFirstReferenceChild(int itemCount) {
        ...
        int firstFoundLinePosition = mFlexboxHelper.getMIndexToFlexLine()[firstFoundPosition];
    ...
    private View findLastReferenceChild(int itemCount) {
        ...
        int lastFoundLinePosition = mFlexboxHelper.getMIndexToFlexLine()[lastFoundPosition];
    ...
    private void recycleFlexLinesFromStart(RecyclerView.Recycler recycler,
        ...
        int currentLineIndex = mFlexboxHelper.getMIndexToFlexLine()[getPosition(firstView)];
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (canViewBeRecycledFromStart(view, layoutState.mScrollingOffset)) {
                if (flexLine.getMLastIndex() == getPosition(view)) {
    ...
    private void recycleFlexLinesFromEnd(RecyclerView.Recycler recycler, LayoutState layoutState) {
        ...
        int currentLineIndex = mFlexboxHelper.getMIndexToFlexLine()[getPosition(lastView)];
        if (currentLineIndex == NO_POSITION) {
            return;
        }
    ...
    private int layoutFlexLineMainAxisHorizontal(FlexLine flexLine, LayoutState layoutState) {
        assert mFlexboxHelper.getMMeasureSpecCache() != null;
        ...
        switch (mJustifyContent) {
            ...
            case JustifyContent.CENTER:
                childLeft = paddingLeft + (parentWidth - flexLine.getMainSize()) / 2f;
                childRight = parentWidth - paddingRight - (parentWidth - flexLine.getMainSize()) / 2f;
                break;
            ...
            case JustifyContent.SPACE_BETWEEN:
                childLeft = paddingLeft;
                float denominator = flexLine.getItemCount() != 1 ? flexLine.getItemCount() - 1 : 1f;
        ...
        for (int i = startPosition, itemCount = flexLine.getItemCount();
            ...
            long measureSpec = mFlexboxHelper.getMMeasureSpecCache()[i];
    ...
    private int layoutFlexLineMainAxisVertical(FlexLine flexLine, LayoutState layoutState) {
        ...
        switch (mJustifyContent) {
            ...
            case JustifyContent.FLEX_END:
                ...
                break;
            case JustifyContent.CENTER:
                childTop = paddingTop + (parentHeight - flexLine.getMainSize()) / 2f;
                childBottom = parentHeight - paddingBottom
                        - (parentHeight - flexLine.getMainSize()) / 2f;
                break;
            case JustifyContent.SPACE_AROUND:
                if (flexLine.getItemCount() != 0) {
    ...
    private void updateLayoutState(int layoutDirection, int absDelta) {
        ...
        if (layoutDirection == LayoutState.LAYOUT_END) {
            ...
            int lastVisibleLinePosition = mFlexboxHelper.getMIndexToFlexLine()[lastVisiblePosition];
            ...
            if (mFlexboxHelper.getMIndexToFlexLine().length <= mLayoutState.mPosition) {
                mLayoutState.mFlexLinePosition = NO_POSITION;
            } else {
                mLayoutState.mFlexLinePosition
                        = mFlexboxHelper.getMIndexToFlexLine()[mLayoutState.mPosition];
            }
            ...
            int firstVisibleLinePosition = mFlexboxHelper.getMIndexToFlexLine()[firstVisiblePosition];
            ...
            mLayoutState.mItemDirection = LayoutState.ITEM_DIRECTION_TAIL;
            int flexLinePosition = mFlexboxHelper.getMIndexToFlexLine()[firstVisiblePosition];
    ...
    private View findFirstReferenceViewInLine(View firstView, FlexLine firstVisibleLine) {
        ...
        for (int i = 1, to = firstVisibleLine.getItemCount();
             i < to; i++) {
    ...
    private int computeScrollOffset(RecyclerView.State state) {
        ...
        int laidOutArea = Math.abs(mOrientationHelper.getDecoratedEnd(lastReferenceView) -
                mOrientationHelper.getDecoratedStart(firstReferenceView));
        int firstLinePosition = mFlexboxHelper.getMIndexToFlexLine()[minPosition];
        if (firstLinePosition == 0 || firstLinePosition == NO_POSITION) {
            return 0;
        }
        int lastLinePosition = mFlexboxHelper.getMIndexToFlexLine()[maxPosition];
    ...
    int getPositionToFlexLineIndex(int position) {
        assert mFlexboxHelper.getMIndexToFlexLine() != null;
        return mFlexboxHelper.getMIndexToFlexLine()[position];
    }
    ...
    private class AnchorInfo {
        ...
        private void assignFromView(View anchor) {
            ...
            int flexLinePosition =
                    mFlexboxHelper.getMIndexToFlexLine()[mPosition != NO_POSITION ? mPosition : 0];
            ...
            if (mFlexLines.size() > mFlexLinePosition) {
                mPosition = mFlexLines.get(mFlexLinePosition).getFirstIndex();
            }
        }
